...
def get_popular_articles():
    ...
    c = db.cursor()
    c.execute("Select title, count from article_popularity" +
              "order by count desc")
    results = c.fetchall()
    db.close()
    return results
...
def get_errors():
    ...
    db = psycopg2.connect(database=DBNAME)
    c = db.cursor()
    c.execute("Select date, percent_total from request_proportions " +
              "where status = '404 NOT FOUND' and percent_total > 0.01")
    results = c.fetchall()
    db.close()
    return results
